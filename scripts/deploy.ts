...
async function main() {
  ...
  try {
    ...
    if (fs.existsSync(deploymentPath)) {
      ...
      if (existingDeployment?.contractAddress) {
        ...
        if (code !== '0x' && !forceDeploy) {
          ...
          console.log(`\n🔄 To force redeploy, use: npx hardhat run scripts/deploy.ts --network localhost --force`);
          return;
    ...
    console.log("No existing deployment found, proceeding with new deployment...");
  }
  ...
  try {
    ...
    console.log("⚠️  Could not save deployment info to file:", error.message);
  }
